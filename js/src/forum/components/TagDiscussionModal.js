...
import DiscussionPage from 'duroom/components/DiscussionPage';
import Button from 'duroom/components/Button';
import LoadingIndicator from 'duroom/components/LoadingIndicator';
import highlight from 'duroom/helpers/highlight';
import classList from 'duroom/utils/classList';
import extractText from 'duroom/utils/extractText';
import KeyboardNavigatable from 'duroom/utils/KeyboardNavigatable';
...
export default class TagDiscussionModal extends Modal {
  ...
  title() {
    return this.attrs.discussion
      ? app.translator.trans('duroom-tags.forum.choose_tags.edit_title', {title: <em>{this.attrs.discussion.title()}</em>})
      : app.translator.trans('duroom-tags.forum.choose_tags.title');
  }
  ...
  getInstruction(primaryCount, secondaryCount) {
    ...
    if (primaryCount < this.minPrimary) {
      ...
      return app.translator.trans('duroom-tags.forum.choose_tags.choose_primary_placeholder', {count: remaining});
    } else if (secondaryCount < this.minSecondary) {
      const remaining = this.minSecondary - secondaryCount;
      return app.translator.trans('duroom-tags.forum.choose_tags.choose_secondary_placeholder', {count: remaining});
    }
        ...
        <div className="TagDiscussionModal-form">
          ...
          <div className="TagDiscussionModal-form-submit App-primaryControl">
            <Button type="submit" className="Button Button--primary" disabled={!this.meetsRequirements(primaryCount, secondaryCount)} icon="fas fa-check">
              {app.translator.trans('duroom-tags.forum.choose_tags.submit_button')}
            </Button>
          </div>
        </div>
      </div>,
        ...
        {!!app.forum.attribute('canBypassTagCounts') && (
          <div className="TagDiscussionModal-controls">
            <ToggleButton className="Button" onclick={() => this.bypassReqs = !this.bypassReqs} isToggled={this.bypassReqs}>
              {app.translator.trans('duroom-tags.forum.choose_tags.bypass_requirements')}
            </ToggleButton>
          </div>
        )}
      </div>
