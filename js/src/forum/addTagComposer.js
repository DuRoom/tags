...
import DiscussionComposer from 'duroom/components/DiscussionComposer';
  ...
  extend(DiscussionComposer.prototype, 'headerItems', function (items) {
    ...
    items.add('tags', (
      <a className={classList(['DiscussionComposer-changeTags', !selectableTags.length && 'disabled'])} onclick={this.chooseTags.bind(this)}>
        {tags.length
          ? tagsLabel(tags)
          : <span className="TagLabel untagged">{app.translator.trans('duroom-tags.forum.composer_discussion.choose_tags_link')}</span>}
      </a>
    ), 10);
  });
