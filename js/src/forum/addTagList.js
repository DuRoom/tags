...
import Separator from 'duroom/components/Separator';
...
export default function() {
  ...
  extend(IndexPage.prototype, 'navItems', function (items) {
    items.add('tags', <LinkButton icon="fas fa-th-large" href={app.route('tags')}>
      {app.translator.trans('duroom-tags.forum.index.tags_link')}
    </LinkButton>
      , -10);
    ...
    if (more.length) {
      items.add('moreTags', <LinkButton href={app.route('tags')}>
        {app.translator.trans('duroom-tags.forum.index.more_link')}
      </LinkButton>, -16)
    }
  });
}
