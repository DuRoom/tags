...
import { slug } from 'duroom/utils/string';
...
export default class EditTagModal extends Modal {
  ...
  title() {
    return this.name()
      ...
      : app.translator.trans('duroom-tags.admin.edit_tag.title');
  }
  ...
  fields() {
    ...
    items.add('name', <div className="Form-group">
      <label>{app.translator.trans('duroom-tags.admin.edit_tag.name_label')}</label>
      <input className="FormControl" placeholder={app.translator.trans('duroom-tags.admin.edit_tag.name_placeholder')} value={this.name()} oninput={e => {
    ...
    items.add('slug', <div className="Form-group">
      <label>{app.translator.trans('duroom-tags.admin.edit_tag.slug_label')}</label>
      <input className="FormControl" bidi={this.slug} />
    </div>, 40);

    items.add('description', <div className="Form-group">
      <label>{app.translator.trans('duroom-tags.admin.edit_tag.description_label')}</label>
      <textarea className="FormControl" bidi={this.description} />
    </div>, 30);

    items.add('color', <div className="Form-group">
      <label>{app.translator.trans('duroom-tags.admin.edit_tag.color_label')}</label>
      <input className="FormControl" placeholder="#aaaaaa" bidi={this.color} />
    </div>, 20);

    items.add('icon', <div className="Form-group">
      <label>{app.translator.trans('duroom-tags.admin.edit_tag.icon_label')}</label>
      <div className="helpText">
        {app.translator.trans('duroom-tags.admin.edit_tag.icon_text', { a: <a href="https://fontawesome.com/icons?m=free" tabindex="-1" /> })}
      </div>
    ...
    items.add('hidden', <div className="Form-group">
      <div>
        <label className="checkbox">
          ...
          {app.translator.trans('duroom-tags.admin.edit_tag.hide_label')}
        </label>
      </div>
    </div>, 10);
    ...
    items.add('submit', <div className="Form-group">
      ...
      }, app.translator.trans('duroom-tags.admin.edit_tag.submit_button'))}
      {this.tag.exists ? (
        <button type="button" className="Button EditTagModal-delete" onclick={this.delete.bind(this)}>
          {app.translator.trans('duroom-tags.admin.edit_tag.delete_tag_button')}
        </button>
      ) : ''}
    </div>, -10);
  ...
  delete() {
    if (confirm(app.translator.trans('duroom-tags.admin.edit_tag.delete_tag_confirmation'))) {
