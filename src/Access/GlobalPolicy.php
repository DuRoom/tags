    ...
    public function can(User $actor, string $ability)
    {
        ...
        if (in_array($ability, ['viewForum', 'startDiscussion'])) {
            if (! isset($enoughPrimary[$actor->id][$ability])) {
                $primaryTagsWhereNeedsPermission = $this->settings->get('duroom-tags.min_primary_tags');
            ...
            if (! isset($enoughSecondary[$actor->id][$ability])) {
                $secondaryTagsWhereNeedsPermission = $this->settings->get('duroom-tags.min_secondary_tags');
